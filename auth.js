  ...
  function renderProfileView() {
    overlay.innerHTML = `
      <div class="armsModal">
        ...
        ${(window.electronAPI && window.electronAPI.isElectron) ? '<button class="logoutBtnModal" id="quitAppBtnModal" style="margin-top:8px;background:linear-gradient(180deg,#2a2f38,#14171c);border-color:rgba(180,190,200,.4);color:#cfd6dd;">✕ Quitter l\'application</button>' : ''}
      </div>
    `;
    ...
    const quitBtnModal = overlay.querySelector('#quitAppBtnModal');
    if (quitBtnModal) {
      quitBtnModal.addEventListener('click', () => {
        if (confirm('Voulez-vous vraiment quitter A’rms ?')) {
